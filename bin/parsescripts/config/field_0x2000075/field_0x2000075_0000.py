...
def entry_0x2000075(infile, field, offset, len, version, mode, outfile):
    ...
    #        return error['ERR_CHECK_LEN']
        ...
        print("enter entry_0x2000075\n")
        (dump_phy_addr,) = config.get_mntn_addr()
        print("dump_phy_addr = 0x%x\n" %(dump_phy_addr))
        pm_om_phy_addr = dump_phy_addr + eval(offset)
        print("pm_om_phy_addr = 0x%x\n" %(pm_om_phy_addr))
...
#        magic = struct.unpack('i', infile.read(4))
        while True:
            (magic, ) = struct.unpack('I', infile.read(4))
            if magic == 0x20424241:
                (dumpdata, ) = struct.unpack('I', infile.read(4))
                (abb_phy_addr, ) = struct.unpack('I', infile.read(4))
                flag += 1
            if magic == 0x20504242:
                (dumpdata, ) = struct.unpack('I', infile.read(4))
                (bbp_phy_addr, ) = struct.unpack('I', infile.read(4))
                flag += 1
            if flag == 2:
                break
#        outfile.writelines('%-30d\n' %(magic))
        print("abb_phy_addr = 0x%x\n" %(abb_phy_addr))
        print("bbp_phy_addr = 0x%x\n" %(bbp_phy_addr))
#        outfile.writelines('abb_phy_addr = %-30x\n' %(abb_phy_addr))
#        outfile.writelines('bbp_phy_addr = %-30x\n' %(bbp_phy_addr))
#        outfile.writelines('flag = %-30d\n' %(flag))
#        outfile.writelines('abb_offset_addr = %-30x\n' %(eval(offset)+abb_phy_addr-pm_om_phy_addr))
#        outfile.writelines('bbp_offset_addr = %-30x\n' %(eval(offset)+bbp_phy_addr-pm_om_phy_addr))
        print("abb_offset_addr = 0x%x\n" %(eval(offset)+abb_phy_addr-pm_om_phy_addr))
        ret = parse_abb_timestamp(infile,eval(offset)+abb_phy_addr-pm_om_phy_addr,outfile)###
        ret = parse_bbp_timestamp('bbp_timestamp',infile,eval(offset)+bbp_phy_addr-pm_om_phy_addr,outfile)###
        return 0    
