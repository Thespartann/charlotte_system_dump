#######################################################################################################################################
#   filename        :   analysis_guphy_dump_info.py
#
...
UPHY_VOS_RATMODE_MAX_NUM                = 6
UPHY_PLATFORM_MAX_RAT_NUM               = 7
UPHY_SPIN_LOCKTYPE_MAX_NUM              = 5
...
def analysis_guphy_modem0_InitCtx_SpinLock_Info( instream, fileOffset, outstream ):
        ...
        while UphySpinLockIdx < UPHY_SPIN_LOCKTYPE_MAX_NUM:
                ...
                outstream.writelines(["%-80s%-2s%-46s\n" % ("\nenSpinLockId:","",strSpinLockId)])

                (uhwRsv,)                       = struct.unpack('H', instream.read(2))
                strRsv                          = '%x'% uhwRsv

                (uwRawLock,)                    = struct.unpack('I', instream.read(4))
                strRawLock                      = '%x'% uwRawLock
                outstream.writelines(["%-80s%-2s%-46s\n" % ("stSpinLockTask.uwRawLock:","0x",strRawLock)])

                (uwOwnerTid,)                   = struct.unpack('I', instream.read(4))
                strOwnerTid                    = '%x'% uwOwnerTid
                outstream.writelines(["%-80s%-2s%-46s\n" % ("stSpinLockTask.uwOwnerTid:","0x",strOwnerTid)])

                (uwOwnerCpu,)                   = struct.unpack('I', instream.read(4))
                strOwnerCpu                     = '%x'% uwOwnerCpu
                outstream.writelines(["%-80s%-2s%-46s\n" % ("stSpinLockTask.uwOwnerCpu:","0x",strOwnerCpu)])

                (uwLockLevel,)                  = struct.unpack('I', instream.read(4))
                strLockLevel                    = '%x'% uwLockLevel
                outstream.writelines(["%-80s%-2s%-46s\n" % ("uwLockLevel:","0x",strLockLevel)])

                (uwLockCount0,)                 = struct.unpack('I', instream.read(4))
                strLockCount0                   = '%x'% uwLockCount0
                outstream.writelines(["%-80s%-2s%-46s\n" % ( "auwLockCount[0]:","0x",strLockCount0)])

                (uwLockCount1,)                 = struct.unpack('I', instream.read(4))
                strLockCount1                   = '%x'% uwLockCount1
                outstream.writelines(["%-80s%-2s%-46s\n" % ( "auwLockCount[1]:","0x",strLockCount1)])
...
def analysis_guphy_modem0_InitCtx_Platform_Info( instream, fileOffset, outstream ):
        ...
        while ModemIdx < UPHY_INIT_MODEM_MAX_NUM:
                ...
                while PlatFormIdx < UPHY_PLATFORM_MAX_RAT_NUM:
                        strPlatFormIdx    = '%x'% PlatFormIdx

                        (enRatList,)      = struct.unpack('H', instream.read(2))
                        strRatList        = UPHY_GetRatType( enRatList )
...
def analysis_guphy_modem0_InitCtx_Active_Status_Info( instream, fileOffset, outstream):
        ...
        while ModemIdx < UPHY_INIT_MODEM_MAX_NUM:
                RatModIdx       = 0
                while RatModIdx < UPHY_VOS_RATMODE_MAX_NUM:
                        (uwActStatus,)                  = struct.unpack('I', instream.read(4))
                        strActStatus                    = '%x'% uwActStatus
                        strModemIdx                     = '%x'% ModemIdx
                        strRatModIdx                    = UPHY_GetRatType( RatModIdx )
...
def analysis_guphy_modemX_Apm_Module_Status_Info( instream, fileOffset, outstream ):
        ...
        strMasterMode           = UPHY_GetRatType( enMasterMode )
        outstream.writelines(["%-100s%-50s\n" % ( "enMasterMode",strMasterMode )])

        (enSlaveMode,)         = struct.unpack('I', instream.read(4))
        strSlaveMode           = UPHY_GetRatType( enSlaveMode )
...
def analysis_guphy_dump_info( infile, offset, outfile):
        ...
        fileOffset = fileOffset + 4
        outstream.writelines(["strModem0LogEndFlg         %-15s\n" % ( strEndTick )])
        ...
        outstream.writelines(["strModem1LogBeginFlg         %-15s\n" % ( strBeginTick )])
        ...
        outstream.writelines(["strModem1LogEndFlg         %-15s\n" % ( strEndTick )])
        fileOffset = fileOffset + 4
        ...
        outstream.writelines(["strModem2LogBeginFlg         %-15s\n" % ( strBeginTick )])
...
def entry_0x220000F(infile, field, offset, len, version, mode, outfile):
        outfile.writelines("============GUPHY dump============\n")
        ########check parameter start#############
        if not field == '0x220000F':
            print('hidis field is ', field)
            outfile.writelines(["strModem1LogEndFlg         %-15s\n" % ( field )])
