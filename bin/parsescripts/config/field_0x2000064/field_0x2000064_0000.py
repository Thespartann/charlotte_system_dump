...
def analyse_reg(sci_fp, dst_fp, field_offset):
    ...
    for i in range(0, 3):
        ...
        if(time==0):
            continue
        print("SCI_EXC_EVENT:"+sci_record_event[event+1], file=dst_fp)
        print("SCI_CLASS_KIND: 0x%08x"%(sciclass& 0xffffffffff), file=dst_fp)
        print("SCI_EXC_TIME: 0x%08x"%(time& 0xffffffffff), file=dst_fp)
        print("SCI_EXC_STATE:"+sci_state[state+1], file=dst_fp)
        print("SCI_PROTOCOL:"+sci_protol[protocol+1], file=dst_fp)
